...
def split_lines(lines, spliter, waa_mask=None, walk_around=False):
    final = []  # create a list
    result_mask = [] # mask_sliced_region_walk_around
    
    for idx, line in enumerate(lines, start=0):  # Iterate through each LineString in lines
        ...
        for i in list(splited.geoms):
            ...
            if (type(i) == LineString) and (not i.is_empty):  # If a geometry is a LineString
                # Add the resulting geometries (either LineStrings or other types) to the final list
                final.append(i)

                if (walk_around == True):
                    if (waa_mask[idx] == 1):
                        result_mask.append(1)
                    else:
            ...
            # If a geometry is not a LineString, print “not linestring” (this is a debugging step)
                ...
                print("Empty linestring or not linestring")

    if (walk_around):
        return final, result_mask
    else:
...
def split_by_regions(lines, regions, layer=None, walk_around=False):
    # create a list of geometries extracted from the MultiLineString (lines)

    if (walk_around == False):
    ...
    elif (walk_around == True):

        for region in regions:
            # Before slice into regions, insert the new "displacement linestrings" generated by the walk_around algorithm
            final = layer.continuous_infill_w_sidewalk
            infill_waa_mask = layer.mask_infill_with_waa
